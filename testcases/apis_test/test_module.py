...
class Test_module_manage():
    def test_creat_module001(self):
        ...
        csurl, method, headers, yamlvalue, yaml_path, mainkey=get_common_params('module.yaml',func_name)
        a = RestClient(rooturl)
        ...
        module_id = res_dir['data']['id']
        return module_id

    def test_delete_module002(self):
        #创建模块后返回模块名称和id
        module_id=Test_module_manage.test_creat_module001(self)
        func_name = sys._getframe().f_code.co_name
        csurl, method, headers, yamlvalue, yaml_path, mainkey = get_common_params('module.yaml', func_name)
        csurl=csurl+module_id
        a = RestClient(rooturl)
